...
most_popular_2013_name <- names_2013$name[names_2013$prop == max(names_2013$prop)]
...
most_popular_in_year <- function(year) {
  filter <- names[names$year == year, ]
  name <- filter$name[filter$prop == max(filter$prop)]
}

# What was the most popular female name in 1994?
most_popular_1994_name <- most_popular_in_year("1994")
...
number_in_million <- function(name, year) {
  filter <- names[names$year == year, ]
  name_prob <- filter$prop[filter$name == name]
  round(1000000 * name_prob,1)
}
